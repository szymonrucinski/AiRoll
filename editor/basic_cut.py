        ...
        if i<len(cut_list)-1:
            ...
            for j,scene in enumerate(cur_shot):
                if len(scene) >= number_of_frames:
                    ...
                    if len(chunk) > 75: continue
                    render_sequence = render_sequence + chunk[0:number_of_frames+1]
                    counter = len(chunk) + counter
                    print('using:{}'.format(cut_list[i]), len(chunk[0:number_of_frames+1]))
                    break
                    # print(number_of_frames,len(chunk))

    check_audio ='C:\\Users\\Szymon\\Downloads\\download.mp3'
