...
def init(video_paths, song_path, progress):
        ...
        if i < len(cut_list) - 1:
            ...
            for j, scene in enumerate(cur_shot):
                if len(scene) >= number_of_frames:
                    ...
                    print(
                        "using:{}".format(cut_list[i]),
                        len(chunk[0 : number_of_frames + 1]),
                    )
                    break
    ...
    clips = [
        ImageClip(frame).set_duration(1 / vc.get_fps()) for frame in render_sequence
    ]
    concat_clip = concatenate_videoclips(clips, method="compose")
    concat_clip.write_videofile(
        os.path.join(BASE_DIR, "render.mp4"),
        audio=new_song_path,
        codec="libx264",
        fps=vc.get_fps(),
    )
